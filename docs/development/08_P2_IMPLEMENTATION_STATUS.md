...
|---|---|---|---|
| CC-P2-001 POS Shift | Implemented | Opening float, cash movements, expected cash, counted close with a required reason for any difference, approval hook for large variances, one open shift per register enforced by a partial unique index | Cash drawer hardware in P6 |
| CC-P2-002 Sale | Implemented | Scan and search from the P1 catalog, cart lines, price, discount and tax, confirmed sale with a readable document number | Favourites and department grids |
...
| CC-P2-004 Hold Sale | Implemented | A cart is held with a name, listed, resumed, edited and confirmed | Discarding a held cart from the POS |
...
1. Quotations and sales orders for non-instant sales (`CC-P2-012`).
2. A payment provider integration so card and wallet tenders resolve themselves.
3. Exchange directly from the POS screen, and discarding a held cart.
4. Offline payments against an already-posted sale, plus conflict comparison in business language.
5. Connected receipt printing, which belongs with the P6 device work.
