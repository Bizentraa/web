...
| Date | Status | SRS mapping | Commit | Summary |
|---|---|---|---|---|
| 2026-08-27 | Implemented for current P5 shared-engine foundation scope | `CC-P5-001` to `CC-P5-012`, `CC-US-012`, `CC-US-013`, `CC-US-014` | `715fe73 feat: add common core P5 P6 engines and reliability` | Added workflow statuses/transitions, work tickets, bookings with overlap prevention, customer assets, traceable units, warranty claims, BOMs, material consumption, delivery routes/stops, notification events, document metadata, P5 permissions/role sync, API/client contracts, Back Office `/business-engines` workspace and live smoke coverage. |
...
| 2026-08-28 | Implemented for Back Office header/table cleanup | Back Office shell and shared table API | This changeset | Removed the `eyebrow` prop from the Back Office `Workspace` API and removed the `kicker` prop from `DataTable`; screen headers now rely on title, description, status and actions, while table headers rely on caption and toolbar. |
...
| Field | Details |
|---|---|
| Feature / slice | Permission catalogue, role templates and existing Business role backfill for P3 |
| Status | Implemented |
| SRS mapping | `CC-P0-006` Roles; `CC-P3-001` to `CC-P3-012`; `CC-US-007`; `CC-US-008`; `CC-US-009` |
...
| Commit | `c3bc01d feat: adopt shadcn sidebar shell` |
...
### 2026-08-28 - Back Office shadcn Interaction Polish

| Field | Details |
|---|---|
| Feature / slice | Back Office interaction polish on the shadcn/Tailwind shell |
| Status | Implemented for current Back Office polish scope |
| SRS mapping | `CC-P0-005` Users; `CC-P0-008` Feature Access; `CC-P1-001` to `CC-P1-011`; `CC-P2-001` Sales; `CC-P2-012` Returns; `CC-US-001`; `CC-US-003`; `CC-US-004`; `CC-US-018` |
| What changed | Moved list search, filter selects and active chips into `DataTable` so controls and rows live in one panel; removed card wrappers around the main Sales, Catalog, Customers and Suppliers tables; converted the Sales shifts tab into a comparable table with a reconciliation drawer for tenders and cash movements; split Import into Import and History tabs; gave every static tab label an explicit icon and added one sliding active indicator for horizontal and vertical tabs; added design-system `Avatar`, inline Role selects and checkbox choice lists for access management; guarded self-administration from the current user's row; rebuilt the Appearance screen from shared components with a real token-scoped preview; unified form control height, themed scrollbars and softened dark border tokens with a cache-version bump; simplified `SkeletonScreen` into one consistent loading shape. |
| Main files | `packages/design-system/src/index.tsx`; `packages/design-system/src/client.tsx`; `packages/design-system/styles.css`; `packages/themes/src/index.ts`; `apps/backoffice/src/app/access/page.tsx`; `apps/backoffice/src/app/appearance/appearance-settings.tsx`; `apps/backoffice/src/app/sales/page.tsx`; `apps/backoffice/src/app/catalog/page.tsx`; `apps/backoffice/src/app/customers/page.tsx`; `apps/backoffice/src/app/suppliers/page.tsx`; `apps/backoffice/src/app/import/page.tsx`; `apps/backoffice/src/app/globals.css`; `apps/backoffice/src/components/app-sidebar.tsx`; `apps/pos/src/app/globals.css`; `docs/development/06_UI_COMPONENT_SYSTEM.md`; `docs/development/15_BACKOFFICE_SIDEBAR_SHADCN.md` |
| Verification | `pnpm prettier --write` for changed code/docs; `git diff --check`; `pnpm --filter @bizentra/design-system build`; `pnpm --filter @bizentra/design-system typecheck`; `pnpm --filter @bizentra/design-system lint`; `pnpm --filter @bizentra/design-system test`; `pnpm --filter @bizentra/themes typecheck`; `pnpm --filter @bizentra/backoffice typecheck`; `pnpm --filter @bizentra/backoffice lint`; `pnpm --filter @bizentra/backoffice test`; `pnpm --filter @bizentra/backoffice build`; `pnpm --filter @bizentra/pos typecheck`; `pnpm --filter @bizentra/pos lint`; `pnpm --filter @bizentra/pos test`; `pnpm --filter @bizentra/pos build`; `pnpm format:check`; `pnpm lint`; `pnpm typecheck`; `pnpm test`. |
| Commit | `68ec1cd feat: polish backoffice shadcn interactions` |
| Remaining work | Branch-scoped API filtering for `/sales`, `/inventory` and `/finance`; real signed-in user endpoint for the sidebar footer; automated visual-regression coverage for expanded, collapsed and mobile sidebar states. |

### 2026-08-28 - POS Cashier Experience Overhaul

| Field | Details |
|---|---|
| Feature / slice | Point of sale: selling, payment, close shift, returns and device responsiveness |
| Status | Implemented for current POS overhaul scope |
| SRS mapping | `CC-P2-001` Shifts and selling; `CC-P2-012` Returns; `CC-P0-001`; `CC-US-004`; `CC-US-018` |
...
| Commit | `812498e feat: refine numeric entry focus` |
| Remaining work | Add browser-level focus assertions once route-level visual tests are introduced. |

### 2026-08-28 - Back Office Header and Table Label Cleanup

| Field | Details |
|---|---|
| Feature / slice | Back Office `Workspace` and shared `DataTable` API simplification |
| Status | Implemented |
| Product area | Back Office shell, Back Office tables, shared design system |
| What changed | Removed `eyebrow` from `Workspace` so page headers no longer require a separate small label above the title. Breadcrumb grouping now comes from the sidebar route map. Removed `kicker` from `DataTable` so every table header has one primary text label, its `caption`, plus any toolbar action. Cleaned the Back Office page callers and removed the unused `.bo-screen-eyebrow` styles. Record-level drawer/entity eyebrows remain because they identify the open record context rather than duplicating page/table labels. |
| Main files | `apps/backoffice/src/app/lib/workspace.tsx`; `packages/design-system/src/index.tsx`; `apps/backoffice/src/app/*/page.tsx`; `apps/backoffice/src/app/globals.css`; `docs/development/03_DEVELOPMENT_CHANGE_LOG.md`; `docs/development/06_UI_COMPONENT_SYSTEM.md`; `docs/development/15_BACKOFFICE_SIDEBAR_SHADCN.md` |
| Verification | `pnpm prettier --write` for changed Back Office code, design-system source and docs; `pnpm --filter @bizentra/design-system build`; `pnpm --filter @bizentra/design-system typecheck`; `pnpm --filter @bizentra/design-system lint`; `pnpm --filter @bizentra/backoffice typecheck`; `pnpm --filter @bizentra/backoffice lint`; `pnpm --filter @bizentra/backoffice test`; `pnpm --filter @bizentra/backoffice build`; `pnpm format:check`; `pnpm lint`; `pnpm typecheck`; `pnpm test`. |
